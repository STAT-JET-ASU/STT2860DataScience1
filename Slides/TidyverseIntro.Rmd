...
It is perfectly possible to do dataset manipulation and plotting using base `R`. However...

* data manipulation can require many intermediate steps

* we often have to nest functions inside of other functions

* it can be difficult to "read" the resulting code to determine what is being done to the data

* the functions for plotting in base `R` have different settings / parameters and can be difficult to recall and manipulate

* the quality of the plots is fine for data exploration but less acceptable for professional presentations and publications


## What is the Tidyverse?

Developed by [Hadley Wickham](https://hadley.nz/) and others, the [tidyverse](https://www.tidyverse.org/) is "an opinionated collection of R packages designed for data science. All packages share an underlying design philosophy, grammar, and data structures."

<hr>

There are eight core packages in the tidyverse. They all will be loaded when you use `library(tidyverse)`. 
...
* all plots use a consistent, underlying "grammar of graphics"
...
The pipe operator (%>%) from the [magrittr](https://magrittr.tidyverse.org/) package allows us to create sequences or "pipelines" of actions/functions rather than nesting functions. 
...
The main difference is that we can now use a pipe without the magrittr package (or tidyverse packages that load it). 
