---
title: "Intro to Data Wrangling<br>with dplyr Verbs, Part 2"
...
* Intro to Data Wrangling with dplyr Verbs, Part 1 [slides](https://stat-jet-asu.github.io/STT2860DataScience1/Slides//DataWranglingVerbs01.html)
...
# Verbs for Observations<br>(Manipulating Rows)
...
Reduce the dataset's rows so it only contains observations for daughters. Note the use of the logical equals `==` and category name in quotes.

```{r}
galton_daughters <- galtondata %>% 
  filter(Child == "Daughter")
glimpse(galton_daughters)
```
...
We use `summarize()` to compute different kinds of quantitative summaries of variables, such as `mean()`. The output is a smaller dataset, which can be further operated on by dplyr verbs. 
...
If we use `group_by()` before `summarize()`, the summaries will be calculated separately for different sub-groupings (groups of rows) in the dataset. Here we have separate summaries for the observations (rows) representing daughters versus sons. Each of the subgroups becomes a row in the output dataset.
...
* Create a subset of families with only one child.

* Extract all the unique `FamilyID`s in the dataset.

* Sort the dataset from the shortest to the tallest child.

* Sort the dataset from the tallest to the shortest child.

* Choose a random sample of observations from the dataset.

* Find all observations where sons are taller than their fathers.

* Subset the data to *include* all observations where the heights of the parents are different by more than 6 inches.

* Subset the data to *exclude* all observations where the heights of the parents are different by more than 6 inches.
